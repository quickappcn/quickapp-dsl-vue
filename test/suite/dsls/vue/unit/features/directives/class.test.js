...
import { firstChild, lastChild } from '../../utils/helper'
...
function assertClass(assertions, done) {
  ...
  const chain = global.waitForUpdate()
  assertions.forEach(([value, expected], i) => {
    chain
      ...
      .then(() => {
        expect(vm.$el.attr.class).to.equal(expected)
...
describe('Directive v-bind:class', () => {
  it('plain string', done => {
    assertClass(
      [['bar', 'foo bar'], ['baz qux', 'foo baz qux'], ['qux', 'foo qux'], [undefined, 'foo']],
      done
    )
  })
  ...
  it('class merge between parent and child', done => {
    ...
    expect(vm.$el.attr.class).to.equal('c a d b')
    vm.value = 'e'
    global
      .waitForUpdate(() => {
        expect(vm.$el.attr.class).to.equal('c a d e')
      })
      .then(() => {
        child.value = 'f'
      })
      .then(() => {
        expect(vm.$el.attr.class).to.equal('c a f e')
      })
      .then(() => {
        vm.value = { foo: true }
        child.value = ['bar', 'baz']
      ...
      .then(() => {
        expect(vm.$el.attr.class).to.equal('c a bar baz foo')
      })
      .then(done)
  })
  ...
  it('class merge between multiple nested components sharing same element', done => {
    ...
    expect(vm.$el.attr.class).to.equal(
      'staticClass componentClass3 componentClass2 componentClass1'
    )
    vm.componentClass1 = 'c1'
    global
      .waitForUpdate(() => {
        expect(vm.$el.attr.class).to.equal('staticClass componentClass3 componentClass2 c1')
        vm.componentClass2 = 'c2'
      })
      .then(() => {
        expect(vm.$el.attr.class).to.equal('staticClass componentClass3 c2 c1')
        vm.componentClass3 = 'c3'
      })
      .then(() => {
        expect(vm.$el.attr.class).to.equal('staticClass c3 c2 c1')
      })
      .then(done)
  })
  ...
  it('deep update', done => {
    ...
    expect(vm.$el.attr.class).to.equal('a')
    vm.test.b = true
    global
      .waitForUpdate(() => {
        expect(vm.$el.attr.class).to.equal('a b')
      })
      .then(done)
  })
  ...
  it('properly remove staticClass for toggling un-keyed children', done => {
    ...
    expect(firstChild(vm.$el).attr.class).to.equal('a')
    vm.ok = false
    global
      .waitForUpdate(() => {
        expect(lastChild(vm.$el).attr.class).to.equal(undefined)
      })
      .then(done)
  })
})
